...
from utils.openai_utils import generate_topics, edit_topics, generate_text, generate_image_prompt
...
def register(bot):
    ...
    def approve_topics(call: CallbackQuery):
        ...
        bot.send_message(
            chat_id,
            "✅ Темы одобрены! Начинаю генерацию постов...",
            reply_markup=None
        )
    ...
    def ask_edit_topics(call: CallbackQuery):
        ...
        bot.send_message(
            chat_id,
            "Опишите, как изменить темы (стиль, направление, конкретные пожелания):"
        )
    ...
    def ask_custom_topics(call: CallbackQuery):
        ...
        bot.send_message(
            chat_id,
            "Напишите 3 темы для постов, каждую с новой строки:"
        )
    ...
    def ask_edit_post(call: CallbackQuery):
        ...
        planning_states[chat_id] = {
            "action": "waiting_post_edit",
        ...
        bot.send_message(
            chat_id,
            "Опишите, как изменить этот пост (стиль, содержание, акценты):"
        )
    ...
    def finish_planning(call: CallbackQuery):
        ...
        message = f"🎯 **Планирование завершено!**\n\n"
        message += f"✅ Одобрено постов: {approved_count}\n"
        message += f"📅 Посты будут опубликованы по расписанию:\n"
        message += f"• Понедельник 19:00 МСК\n"
        message += f"• Среда 19:00 МСК\n"
        message += f"• Пятница 19:00 МСК\n\n"
        message += f"Используйте /admin для управления."
...
def _handle_custom_topics(bot, msg: Message):
    ...
    if len(topics) < 3:
        bot.send_message(
            chat_id,
            f"Нужно минимум 3 темы, получено: {len(topics)}. Попробуйте еще раз."
        )
        return
...
def _generate_posts_for_topics(bot, chat_id: int):
    ...
    try:
        ...
        if posts:
            ...
            bot.send_message(
                chat_id,
                f"✅ Сгенерировано {len(posts)} постов! Начинаем согласование..."
            )
...
def _show_next_post_or_finish(bot, chat_id: int, current_index: int):
    ...
    if next_index < len(pending_posts):
        ...
        message = f"🎯 **Все посты просмотрены!**\n\n"
        message += f"✅ Одобрено постов: {approved_count}\n\n"
