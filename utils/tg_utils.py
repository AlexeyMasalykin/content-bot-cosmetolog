...
def send_post_with_image(bot, chat_id, text: str, image_bytes: bytes, reply_markup=None, parse_mode=None):
    ...
    if len(text) <= 1000:
        try:
            return bot.send_photo(
                chat_id, io.BytesIO(image_bytes), caption=text, reply_markup=reply_markup, parse_mode=parse_mode
            )
        except Exception:
    ...
    try:
        ...
        return bot.send_message(
            chat_id, f"❌ Ошибка отправки изображения\n\n{text}", reply_markup=reply_markup, parse_mode=parse_mode
        )
...
def topics_approval_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton("📝 Написать свои темы", callback_data="custom_topics"))
    return kb
